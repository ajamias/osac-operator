...
func (f *controllerFlags) enableAllIfNoneSet() {
	if !f.Tenant && !f.HostPool && !f.ComputeInstance && !f.Cluster && !f.Networking && !f.BareMetal {
		f.Tenant = true
		f.HostPool = true
...
func addSchemesForLocalControllers(
	...
	enableCluster, enableHostPool, enableComputeInstance, enableTenant, enableNetworking, enableBareMetal bool,
) {
...
func setupBareMetalControllers(mgr mcmanager.Manager) error {
	...
	lockConfigFilePath := helpers.GetEnvWithDefault(envLockConfigFile, "/etc/osac/lock.yaml")
	...
	// 3. Setup Host controller with dependencies
	if err := (&controller.HostReconciler{
		...
		return fmt.Errorf("host controller: %w", err)
	}
